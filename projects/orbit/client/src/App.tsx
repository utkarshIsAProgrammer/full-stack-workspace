...
export default function App() {
	...
	const setOpusBitrate = (sdp: string): string => {
		...
		return sdp.replace(
			regex,
			`a=fmtp:${pt} maxaveragebitrate=64000;useinbandfec=1`,
		);
	};
		...
		const timer = setInterval(async () => {
			try {
				...
				if (availableBitrate > 0) {
					...
					if (
						Math.abs(newMax - lastMaxBitrate) >
						lastMaxBitrate * 0.1
					) {
						params.encodings[0].maxBitrate = newMax;
						await sender.setParameters(params);
						logger.info(
							`[Bitrate] Adjusted to ${Math.round(newMax / 1000)} kbps (available: ${Math.round(availableBitrate / 1000)} kbps)`,
						);
		...
		async (
			...
			try {
				...
				if (type === "video" && videoTracks.length > 0) {
					...
					if (videoCodec) {
						try {
							(videoTransceiver as any).setCodecPreferences([
								videoCodec,
							]);
						} catch {
				...
				<AnimatePresence mode="wait">
					{sessionChecked && !user && !publicFeedMode ? (
						...
						<motion.div
							...
							<div className="fixed top-3 right-3 z-[140] flex flex-col gap-3.5 w-[calc(100vw-1.5rem)] max-w-sm pointer-events-none sm:top-6 sm:right-6 sm:w-full">
								<AnimatePresence>
							...
							<main className="grow min-h-0 overflow-hidden flex items-stretch justify-center py-0 lg:py-6 w-full h-full">
								{(user || publicFeedMode) && (
				...
				<AnimatePresence>
					{toastMessage && (
						<motion.div
							...
							role="status"
							aria-live="polite"
							className="fixed top-8 left-1/2 z-[160] flex items-center gap-2 bg-zinc-950/90 border border-white/10 px-4 py-2.5 rounded-full shadow-2xl backdrop-blur-md text-xs text-white">
							{toastType === "success" ? (
