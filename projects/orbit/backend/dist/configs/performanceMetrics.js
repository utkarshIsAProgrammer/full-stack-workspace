...
class PerformanceMetrics {
    ...
    async recordMetric(name, value, tags) {
        try {
            ...
            await redis_1.redis.lpush(key, JSON.stringify(data));
            await redis_1.redis.expire(key, 86400);
            // Keep only last 1000 data points
            await redis_1.redis.ltrim(key, 0, 999);
        }
        catch (error) {
     ...
     * Get metric data
     */
    async getMetric(name, limit = 100) {
        try {
            const key = `${this.metricsPrefix}${name}`;
            const data = await redis_1.redis.lrange(key, 0, limit - 1);
     ...
     */
    async recordApiResponse(endpoint, method, duration, statusCode) {
     ...
     */
    async recordDbQuery(collection, operation, duration) {
     ...
     */
    async recordCacheHit(cacheName, hit) {
     ...
     */
    async getAverageMetric(name, timeRange = 3600) {
        try {
     ...
     */
    async getPercentileMetric(name, percentile = 95, timeRange = 3600) {
        try {
            const data = await this.getMetric(name, 1000);
            const now = Date.now();
            const cutoff = now - (timeRange * 1000);
...
const performanceMonitoring = () => {
    return async (req, res, next) => {
        ...
        res.on('finish', () => {
            ...
            performanceMetrics.recordApiResponse(req.path, req.method, duration, res.statusCode);
        });
        next();
    };
};
